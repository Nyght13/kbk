...
int compare_area args((const void * v1,
    const void * v2));
...
void easy_induct(CHAR_DATA * ch, char * argument) {
    ...
    } else if (cabal == CABAL_BOUNTY) {} else if (cabal == CABAL_KNIGHT) {
    ...
    if (!str_cmp(arg1, "all")) {
        ...
        for (parea = area_first; parea != NULL; parea = parea - > next) {
            ...
            for (iHash = 0; iHash < MAX_KEY_HASH; iHash++) {
                for (from_room = room_index_hash[iHash]; from_room != NULL; from_room = from_room - > next) {
                    /*
...
void do_info(CHAR_DATA * ch, char * argument) {
    ...
    if (!str_cmp(arg, "type")) {
        ...
        else { // No obj type found in this area, better print it.
...
int compare_area(const void * v1,
    const void * v2) {
