...
extern int flag_lookup args((const char * name,
                ...
                case 'E':
                    ...
                    if (pMob - > pShop) {
                        switch (pObj - > item_type) {
                            default: pObj - > level = UMAX(0, pObj - > level);
                            break;
                        case ITEM_PILL:
                            case ITEM_POTION:
...
void convert_object(OBJ_INDEX_DATA * pObjIndex) {
    ...
    switch (pObjIndex - > item_type) {
        default: bug("Obj_convert: vnum %d bad type.", pObjIndex - > item_type);
        break;
...
void load_improgs(FILE * fp) {
    ...
    for (;;) {
        ...
        switch (letter = fread_letter(fp)) {
            default: bug("Load_improgs: letter '%c' not *IMS.", letter);
            exit(1);
