 ...
 */
#
define CD CHAR_DATA# define OD OBJ_DATA
...
void do_pour(CHAR_DATA * ch, char * argument) {
    ...
    in - > value[1] += amount;
    out - > value[1] -= amount; in - > value[2] = out - > value[2];
        ...
        if (ch - > level < staff - > level ||
            ...
            for (vch = ch - > in_room - > people; vch; vch = vch_next) {
                ...
                switch (skill_table[sn].target) {
                    default: bug("Do_brandish: bad target for sn %d.", sn);
                    return;
