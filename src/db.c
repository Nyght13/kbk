...
char * flag_name_lookup args((long bitv,
 ...
 */
#
define MAX_STRING 7500000 /* Original was 1483120 and then 3593120 */ # define MAX_PERM_BLOCK 131072# define MAX_MEM_LIST 12
 ...
 */
#
if defined(KEY)# undef KEY# endif
...
void load_resets(FILE * fp) {
    ...
    for (;;) {
         ...
         */
        switch (letter) {
            default: bug("Load_resets: bad command '%c'.", letter);
            exit(1);
            break;
...
void load_specials(FILE * fp) {
    for (;;) {
        ...
        switch (letter = fread_letter(fp)) {
            default: bug("Load_specials: letter '%c' not *MS.", letter);
            exit(1);
                ...
                if (!pObjIndex - > new_format)
                    ...
                    case ITEM_WEAPON:
                            if (pReset - > command == 'G')
                                ...
                                olevel = number_fuzzy(level);#endif /* envy version */
 ...
 *   (compatibility with historical functions).
 */
bool str_prefix(const char * astr,
    const char * bstr) {
    if (astr == NULL) {
 ...
 *   (compatibility with historical functions).
 */
bool str_infix(const char * astr,
    const char * bstr) {
    int sstr1;
    int sstr2;
    int ichar;
 ...
 *   (compatibility with historical functions).
 */
bool str_suffix(const char * astr,
    const char * bstr) {
    int sstr1;
    int sstr2;
