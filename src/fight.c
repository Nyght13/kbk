...
void do_murder(CHAR_DATA * ch, char * argument) {
    ...
    if (IS_NPC(ch))
        ...
        switch (number_range(1, 6)) {
            default: sprintf(buf, "Help!  I am being attacked by %s!", PERS(ch, victim));
            break;
        case (1):
        ...
        for (paf = ch - > affected; paf != NULL; paf = paf_next) {
            ...
            if (paf - > type == sn) {
                switch (paf - > location) {
                    default: break;
                case APPLY_HIT:
                        hitmod = paf - > modifier;affect_remove(ch, paf);
                    break;
                case APPLY_MANA:
                        manamod = paf - > modifier;affect_remove(ch, paf);
                    break;
                case APPLY_HITROLL:
                        hrollmod = paf - > modifier;affect_remove(ch, paf);
                    break;
                case APPLY_DAMROLL:
                        drollmod = paf - > modifier;affect_remove(ch, paf);
                    break;
                }
            }
        }
    ...
    for (af = obj - > affected; af != NULL; af = af_next) {
        ...
        if (af - > type == sn) {
            switch (af - > location) {
                default: break;
            case APPLY_HIT:
                    hpmod = af - > modifier;affect_remove_obj(obj, af);
                break;
            case APPLY_MANA:
                    mmod = af - > modifier;affect_remove_obj(obj, af);
                break;
            case APPLY_HITROLL:
                    hmod = af - > modifier;affect_remove_obj(obj, af);
                break;
            case APPLY_DAMROLL:
                    dmod = af - > modifier;affect_remove_obj(obj, af);
                break;
            }
        }
    }
