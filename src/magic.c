...
void do_cast(CHAR_DATA * ch, char * argument) {
    ...
    switch (skill_table[sn].target) {
        default: bug("Do_cast: bad target for sn %d.", sn);
        return;
...
void obj_cast_spell(int sn, int level, CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * obj) {
    ...
    switch (skill_table[sn].target) {
        default: bug("Obj_cast_spell: bad target for sn %d.", sn);
        return;
...
void spell_remove_curse(int sn, int level, CHAR_DATA * ch, void * vo, int target) {
    ...
    for (obj = victim - > carrying;
        (obj != NULL && !found); obj = obj - > next_content) {
        if ((IS_OBJ_STAT(obj, ITEM_NODROP) || IS_OBJ_STAT(obj, ITEM_NOREMOVE)) &&
...
void spell_energy_drain(int sn, int level, CHAR_DATA * ch, void * vo, int target) {
    ...
    switch (number_range(0, 3)) {
        default: type = 1;amount = dice(level, 3);
        break;
    case (0):
        case (1):
            /* HP */ type = 1;amount = dice(level, 4);
        break;
    case (2):
            /* move */ type = 2;amount = dice(level, 3);
        break;
    case (3):
            /* mana */ type = 3;amount = dice(level, 3);
        break;
    }
