            ...
            for (exits = number_range(0, 5); !thrownOut && throw_attempts < throw_attempts_max; exits = number_range(0, 5)) {
                ...
                if (ex) {
                    if (!IS_SET(ex - > exit_info, EX_CLOSED)) {
                        ...
                        case DIR_NORTH:
                            do_north(rch, "");
                            if (rch - > in_room - > vnum != was_in) thrownOut = TRUE;
                            else break;
...
void spell_vorpal_bolt(int sn, int level, CHAR_DATA * ch, void * vo, int target) {
    ...
    switch (type) {
    case 0:
        if (!is_affected(victim, skill_lookup("vorpal bolt head"))) found = FALSE;
        gsn = skill_lookup("vorpal bolt head");
        break;
    case 1:
        if (!is_affected(victim, skill_lookup("vorpal bolt leg"))) found = FALSE;
        gsn = skill_lookup("vorpal bolt leg");
        break;
    case 2:
        if (!is_affected(victim, skill_lookup("vorpal bolt arm"))) found = FALSE;
        gsn = skill_lookup("vorpal bolt arm");
        break;
    case 3:
        if (!is_affected(victim, skill_lookup("vorpal bolt chest"))) found = FALSE;
