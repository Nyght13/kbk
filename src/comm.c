...
#include < signal.h > #include < fcntl.h > #include < netdb.h > #include < netinet / in .h > #include < sys / socket.h > #include "telnet.h"
 ...
 */
#
if defined(sun)# undef MALLOC_DEBUG# endif

#
if defined(MALLOC_DEBUG)# include < malloc.h >
...
void init_descriptor(int control) {
    ...
    #
    if !defined(FNDELAY)# define FNDELAY O_NDELAY# endif
...
bool output_buffer(DESCRIPTOR_DATA * d) {
    ...
    while ( * str != '\0' && (str - d - > outbuf) < d - > outtop) {
        ...
        case '-':
            ...
            break;
        case '<':
                color = o_color;bold = o_bold;flash = o_flash;
            act = TRUE;
            break;
        case '0':
                color = 0;act = TRUE;
        ...
        case 'n':
                if (d - > character && IS_ANSI(d - > character))
                    ...
                    buf2[0] = '\0';
            bold = FALSE;color = 7;flash = FALSE;
            break;
        }
        if (act) {
            buf2[0] = '\0';
...
void nanny(DESCRIPTOR_DATA * d, char * argument) {
    ...
    switch (d - > connected) {
        default: bug("Nanny: bad d->connected %d.", d - > connected);
        close_socket(d);
        return;
...
void colorconv(char * buffer,
    const char * txt, CHAR_DATA * ch) {
...
void reverse_txt(char * txt, int length) {
    ...
    int i, state, c;
    char * word_ptr = NULL;#
    define WALKTHRU 0# define INWORD 1# define INESC 2
    ...
    for (i = 0, state = WALKTHRU; i < length; i++, txt++) {
        ...
        case WALKTHRU:
            ...
            break;
        case INESC:
            /* ignore contents of color code seq */
