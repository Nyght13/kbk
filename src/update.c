...
void weather_update(void) {
    ...
    switch (weather_info.sky) {
        default: bug("Weather_update: bad sky %d.", weather_info.sky);
...
void obj_update(void) {
    ...
    for (obj = object_list; obj != NULL; obj = obj_next) {
        ...
        if (obj - > item_type == ITEM_CORPSE_PC && obj - > contains) { /* objects go to pit */
            OBJ_DATA * t_obj, * next_obj, * pit_obj;

            /* Put items into the pit that the player recalls to */
            if (IS_EXPLORE(obj - > in_room)) { //Gear to char
